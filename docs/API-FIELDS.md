...
| 공동주택 **실거래가격지수** (월) | `A_2024_00178` | **[확정]** 2026-09-24 실호출 성공 |

통계표 목록 엔드포인트(`easyStatMobileList.do`)로 634개를 받아 확보했다. 정식 명칭은
**"(월) 지역별 매매지수_아파트"**, 경로는 `공동주택 실거래가격지수 > 아파트`다.
기존 두 개도 명칭을 확인했다 — `(월) 매매가격지수_아파트`, `(월) 전세가격지수_아파트`
(`전국주택가격동향조사 > 월간동향 > 아파트`).

> ⚠️ **이 표는 `CLS_ID` 체계가 다른 두 표와 다르다.** 아래 "지역 코드" 절을 반드시 볼 것.
...
|---|---|---|
| `INFO-000` | 정상 | [확정] |
| `INFO-200` | 해당하는 데이터가 없습니다 | **[확정]** 2026-09-24 |
| `ERROR-290` | 인증키가 유효하지 않습니다 | [확정] |
| 전체 코드표 | — | **[미확인]** 개발가이드에 없음 |

> **`INFO-200`은 에러가 아니다.** 접두어가 `INFO-`이고, 조회 조건에 맞는 행이 없을 때 온다.
> 응답은 래퍼 없이 `{"RESULT":{"CODE":"INFO-200",...}}` 단독이다.
> 공표 지연 때문에 최근 1~2개월을 요청하면 정상적으로 이게 온다(매매·전세는 1개월,
> 실거래는 2개월 지연). **빈 결과로 다뤄야 한다** — 에러로 올리면 유효한 요청이 502가 된다.
>
> 주의: **존재하지 않는 `STATBL_ID`에도 같은 `INFO-200`이 온다.** 둘을 구분할 수 없다.
> 통계표 상수 3개가 실측으로 확정됐으므로 받아들인다 — `STATBL_ID`는 런타임 입력이 아니라
> 코드 상수라, 오타가 들어오는 경로가 "누가 `lib/config.ts`를 고칠 때" 하나뿐이다.
>
> **그래서 가드는 코드가 아니라 절차다. 통계표 상수를 추가·변경하면 `months=120`으로 한 번
> 호출해 행이 오는지 확인해라.** 120개월이면 공표 지연으로 0건일 수 없으므로,
> `INFO-200`이 오면 상수가 틀린 것이다.

## 샘플모드 동작 [확정 — 실측, 2026-09-24 보강]
...
**샘플모드도 `START_WRTTIME`/`END_WRTTIME`를 지킨다** — "기간과 무관한 고정 표본"이 아니다.
범위를 주면 그 범위 안에서 앞 5건을 준다. 그래서 요청 기간으로 잘라내도 걸러지지 않는다.

**판별자는 `head`의 `list_total_count`와 실제 행 수의 차이다.** 샘플모드는 총건수를
정직하게 보고하면서 행만 자른다.

```
샘플 + 범위 202504~202509  →  list_total_count: 6 | 실제 행: 5
실키 + 범위 202504~202509  →  list_total_count: 6 | 실제 행: 6
```

**단, 실제 건수가 5 이하이면 두 응답이 완전히 같아 구분할 수 없다.**

```
범위 202604~202609 (202609 미공표):
  실키 → list_total_count: 5 | 행: 5
  샘플 → list_total_count: 5 | 행: 5
```

**"`Type=json`인데 에러가 xml로 온다"는 이슈는 이번 조사에서 재현되지 않았다** [미확인].
정상·오류 응답 모두 JSON으로 왔다. xml 폴백은 방어 코드로만 유지한다.

## 지역 코드 (`CLS_ID`) — **통계표마다 다르다** [확정 — 2026-09-24]

> 🛑 **같은 `CLS_ID`가 통계표에 따라 다른 지역을 가리킨다.** 17개 중 16개가 어긋난다.
> `500008`은 매매·전세 표에서 서울이고 실거래 표에서 **부산**이다. 하나의 매핑을 모든
> 통계표에 쓰면 조용히 다른 지역 데이터가 나온다 — 에러도 안 나고 값도 그럴듯하다.
> `clsIdFor(region, statblId)`처럼 통계표를 함께 받아야 한다.

**`A_2024_00045`(매매) · `A_2024_00050`(전세) — 동일**
...
두 소스(통계코드 검색 엔드포인트 + 실제 API 응답의 `CLS_NM`)로 교차 검증했다.

**통계표 목록·분류 코드를 받는 엔드포인트** (통계코드 검색 화면이 쓰는 것. 화면은 JS 동적
렌더링이라 스크래핑이 안 되지만 이 두 개는 POST로 직접 호출된다):

```
POST /r-one/portal/stat/easyStatMobileList.do
     searchVal=&page=1&rows=1000&cateYn=Y      → 통계표 634개 (statblId, statblNm, fullPath)

POST /r-one/portal/openapi/selectOpenApiItmCd.do
     statblId=A_2024_00178&page=1&rows=1000    → 그 표의 분류(CLS)·항목(ITM) 코드
```

**공표 지연**: 매매·전세 1개월, 실거래 2개월. 실거래는 `200601`부터 247건, 매매·전세는
`200311`부터 274건이다. **세종은 166건**으로 짧다(2012년 출범).
...
- [x] **공동주택 실거래가격지수 `STATBL_ID`** → `A_2024_00178`
- [x] **시도별 `CLS_ID` 17개** → 확보. **단 통계표마다 다르다**("지역 코드" 절)
- [x] 세종 / 강원 / 제주 축약형 실제 표기 → `세종`·`강원`·`제주` 그대로
- [x] 전남광주통합특별시 반영 여부 → 반영됨. 광주·전남이 `"전남광주>광주"`로 한 단계 내려갔다.
      `CLS_ID` 직접 매핑이라 우리 코드에는 영향 없음
- [x] `START_WRTTIME` / `END_WRTTIME` 실제 동작 + 값 형식 → **동작한다**(`YYYYMM`)
- [x] sample 감지 임계값 검증 → **기준 자체가 틀렸다.** `months` 비교는 공표 지연을 오탐한다.
      `list_total_count` vs 실제 행 수로 교체(DESIGN.md §4.5①)
- [x] `RESULT.CODE` 전체 목록 → 전체는 여전히 미공개. `INFO-200`(데이터 없음)이 추가로 확인됐다
