...
**지오코딩을 막는 것은 `일원`이다.** 카카오는 말미 괄호를 스스로 무시한다 —
`경기도 시흥시 하중동 (A블록) (임대)`, `부산광역시 강서구 명지동 (명지국제신도시)` 모두
원문 그대로 `OK`다. 그런데도 괄호를 **먼저** 떼는 이유는 `일원$` 정규식이 앵커이기 때문이다.
괄호가 뒤에 붙어 있으면 `일원`이 말미에 노출되지 않아 제거 자체가 불가능하다.
...
| 층 | 건수 | 내용 |
|---|---|---|
| 실제 주소에서 관측된 `… 일원` 형태 | **1건** | 시흥 하중동 (픽스처 7건 중) |
| `일원`이 지오코딩을 막는다는 기전을 확인한 문자열 | **2건** | 시흥(실측) + `경기도 화성시 동탄면 일원`(합성) |

기전이 한 문자열에만 의존하지 않는다는 것은 합성 대조군으로 충분히 보인다. 다만
**"지구 단위 공고에 이 형태가 얼마나 흔한가"는 여전히 n=1**이고 합성 문자열은 그 질문에
답하지 못한다. 위 표의 `하중동 (A블록) (임대)`·`명지동 (명지국제신도시)`도 합성 프로브다.
성공한 나머지 픽스처(청주·위례·동탄)는 전처리가 문자열을 건드리지도 않아 전처리에 대해
아무것도 증명하지 않는다. 다른 형태가 실데이터에 얼마나 되는지는 Phase 8에서 확인한다.

**픽스처 주의**: `경기도 성남시 분당구 판교역로 123`(URBTY, `2026410099`)은 dev 전용이 아닌
일반 픽스처인데도 지도가 뜨지 않는다. 실재하지 않는 번지라 `ZERO_RESULT`가 나는 것이고,
설계대로 섹션이 숨는 것이다 — 버그가 아니다.
