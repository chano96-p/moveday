...
**세 코드 모두 실제 호출로 확정됐다**(Phase 8). 실거래가격지수는 통계표 목록 엔드포인트
(`easyStatMobileList.do`)로 확보했다 — 통계코드 검색 화면은 JS 동적 렌더링이라 스크래핑이
안 되지만 그 화면이 쓰는 POST 엔드포인트는 직접 호출된다(`API-FIELDS.md` (C) 참조).

> ⚠️ **`CLS_ID` 체계가 통계표마다 다르다.** 매매·전세는 같고 실거래는 별도다 — 같은
> `500008`이 서울(매매·전세)과 부산(실거래)으로 갈린다. `clsIdFor(region, statblId)`가
> 통계표를 필수 인자로 받는 이유다(§4.5④).

코드가 `null`이면 해당 라우트가 `503 REBSTAT_TABLE_UNKNOWN` 을 낸다. 지금은 셋 다 채워져
있어 이 경로는 통계표를 새로 추가하다 오타가 났을 때만 발화한다.
...
#### ⑥ 주택형 표기 — 함정이 아니었다 (2026-09-24 정정)

Phase 1~7은 `"084.9500A"`와 `"55㎡O"` 두 형태가 혼재한다고 보고 `normalizeHouseType()`으로
선행 숫자 + 말미 영문 접미사를 뽑아 정규화했다. **그 관측이 틀렸다.**

- `㎡` 표기는 API에 실재하지 않는다 — Mdl 300행 + 경쟁률 300행(고유값 345개) 전수 0건.
  `API-FIELDS.md`가 청약홈 **웹사이트 렌더링**을 API 실측으로 잘못 기록한 것이었다.
- 정규화가 조인 매칭을 **하나도 늘리지 않았다**: MODEL_NO 있는 오퍼레이션 300/300,
  폴백 조인 대상인 REMNDR(04) 249/249, OPT 291/291 — 원문 비교와 결과가 같다.
- 오히려 정규화가 충돌을 **만들었다**. `TP`(URBTY/RENT)는 `84A-1`·`84A1`처럼 숫자로 끝나는데,
  접미사 정규식 `([A-Za-z]+)\s*$`가 매칭에 실패해 고유 TP 124개가 키 106개로 뭉갰다
  (`"84"` ← `84, 84A1, 84A2, 84A-1, 84B-1, 84C-1` 6중 충돌 등 10건).

→ `toHouseTypeKey()`는 **원문 트리밍만** 한다. 원문은 `houseType`에 보존해 화면에 그대로 보여준다.

**이 축이 안전한 진짜 이유는 표본 크기가 아니라 실패 방향의 비대칭이다.** 두 서비스
(`ApplyhomeInfoDetailSvc` / `ApplyhomeInfoCmpetRtSvc`)가 같은 원본을 본다는 것은 추론이고,
300/300은 한 시점 페이지 1의 표본이다. 그 추론이 틀려도 결론은 바뀌지 않는다:

| 조인이 어긋났을 때 | 결과 |
|---|---|
| 원문 비교 | 매칭 실패 → **경쟁률 열이 숨는다**. §4.4가 이미 허용한 동작 |
| 정규화 | 다른 주택형 6종이 `"84"`로 뭉개져 **다른 주택형의 경쟁률이 붙는다**. §9 위반 |

정규화는 **거짓 매칭을 만들 수 있었고**, 원문 비교는 **누락만 만든다.** 안전한 쪽이다.

**`.trim()`은 실제로 일한다 — 없으면 조인이 깨진다.** 실응답 고유 주택형 743개 중
**138개(18.6%)가 말미 공백을 달고 온다**(`"114.8419 "`, `"069.7032 "` …).
그리고 그 138개는 **말미 영문 접미사가 없는 값과 정확히 일치한다** — 옛 정규화의 충돌이
그렇게 컸던 이유가 이것이다. 접미사 정규식이 `''`를 뽑아 정수부만 남기니 `"114"`류 키가
대량으로 생겼다. 소문자는 743개 중 0개라 대소문자 정규화는 불필요하다.
...
→ **상류가 행을 잘랐으면 키 미적용으로 간주해 명시적 에러**를 던진다.

```ts
if (rows.length < listTotalCount) throw new RebstatSampleResponseError()
```

이 판정은 2026-09-24 실측으로 다시 세운 것이다. **Phase 5까지 쓰던 기준
(`rows.length <= 5 && months > rows.length`, clamp를 먼저 실행)은 전제가 두 군데 틀렸다.**

**(1) 샘플 모드도 `START_WRTTIME`/`END_WRTTIME`를 지킨다.** "요청 기간과 무관한 고정 표본"이
아니다. 범위를 주면 그 범위 안에서 앞 5건을 돌려준다 — 그래서 `clampToRange`로는 걸러지지 않는다.

```
샘플 + 범위 202504~202509  →  list_total_count: 6 | 실제 행: 5 (202504~202508)
실키 + 범위 202504~202509  →  list_total_count: 6 | 실제 행: 6
```

**(2) `months`와 건수를 비교하는 기준은 공표 지연을 오탐한다.** 매매·전세의 최신 공표는
1개월, 실거래는 2개월 지연된다. `months=6`이면 창에 5개월치만 존재해
`5 <= 5 && 6 > 5` → 정상 요청에 `REBSTAT_SAMPLE_RESPONSE`가 난다. §5가 `months` 1~120을
계약으로 명시했는데 7 이하가 전부 502였다.

**진짜 판별자는 `list_total_count`와 실제 행 수의 비교다.** 샘플 모드는 총건수를 정직하게
보고하면서 행만 5개로 자른다. `rows.length < list_total_count`이면 잘린 것이고, 같으면
그게 전부인 것이다 — 공표 지연에는 원리적으로 오탐하지 않는다.

**그리고 원리적으로 구분 불가능한 구간이 있다.** 실제 건수가 5 이하이면 두 응답이 동일하다:

```
오늘(2026-09) 기준 months=6 범위(202604~202609):
  실키 → list_total_count: 5 | 행: 5 | 202604~202608
  샘플 → list_total_count: 5 | 행: 5 | 202604~202608    ← 완전히 같다
```

이 구간은 **데이터를 돌려주는 쪽**으로 정했다. `assertRebstatKey()`가 키 부재를 호출 전에
막고 잘못된 키는 `ERROR-290`으로 따로 걸리므로, 우리 클라이언트가 샘플 모드에 빠지는 경로
자체가 사실상 없다. 그 잔여 위험보다 정상 요청에 틀린 진단을 내는 비용이 크다.

정확히 말하면 이건 선택이 아니라 **자동으로 성립한다** — 샘플 응답도 `list_total_count: 5,
행: 5`라 `5 < 5`가 거짓이다. "왜 안 잡히나"를 다시 조사하지 말 것.

**페이지 한도도 함께 봐야 한다.** 한 번만 호출하고 페이지를 넘기지 않으므로(`pSize=300`),
시계열이 한도를 넘으면 상류는 정상적으로 한도만큼만 준다. 그걸 "잘렸다"로 읽으면 정상 응답이
502가 된다 — `clampToRange`를 남겨둔 이유와 정면으로 모순되는 실패다.

```ts
rowCount < Math.min(listTotalCount, pageSize)
```

오늘은 발화하지 않는다(매매·전세 274건, 한도까지 26개월 여유). 발화 조건은 **월 1행씩 늘어
약 26개월 뒤**, 또는 **주간 주기 통계표를 추가하는 순간**이다.
...
#### ⑤ 기간 파라미터 — 동작한다 (2026-09-24 실측으로 확정)

`START_WRTTIME` / `END_WRTTIME`(`YYYYMM`)은 **실제로 동작한다.** 실키로
`START_WRTTIME=202504&END_WRTTIME=202509`를 주면 정확히 6건이 온다.

그래도 `clampToRange`는 **남긴다.** clamp는 범위 밖 행을 제거만 하고 값을 지어내지 않는다 —
상류가 언젠가 필터를 무시해도 결과가 틀리지 않는 방향이다. 비용은 배열 필터 하나다.

통계표 전체 건수가 56,148건이므로 `CLS_ID` + `ITM_ID` + `DTACYCLE_CD` 필터는 여전히 필수다.
...
| 주택형 조인 키 | `parse.test.ts` | 원문 트리밍(`"074.9976 "`), 숫자-끝 접미사 충돌 회귀(`84A1`/`84A-1`) |
| 자리표시자 | `parse.test.ts` | `"-"`·`""`·`null` → 의미 없음(`GP` 필터링) |
| 유형별 매핑 | `adapters.test.ts` | 5개 유형 픽스처 → `Notice` (접수 윈도우 개수 포함) |
| 지역 매핑 | `region.test.ts` | 17개 화이트리스트, 미매핑 → `null`, R-ONE `CLS_FULLNM` 최상위 추출 |
| sample 응답 감지 | `rebstat-sample.test.ts` | `rows < list_total_count` → throw / 같으면 통과(공표 지연 오탐 회귀) |
...
> **2026-09-24 개정.** Figma 디자인(`pIdGgk2mako96bPQgyWuI4`)을 반영해 무드를 바꿨다.
> Phase 1~8의 "뉴트럴 그레이 단색" 기조는 폐기됐다 — 아래가 현행이다.

### 테마

라이트 테마 단일. 토큰은 `app/globals.css`의 Tailwind v4 `@theme`에만 둔다 —
**화면 코드에 hex를 직접 쓰지 않는다.** 디자인이 바뀌면 여기만 고친다.

| 토큰 | 값 | 용도 |
|---|---|---|
| `canvas` / `surface` / `field` | `#f7f7fa` / `#ffffff` / `#f8fafc` | 페이지 / 카드 / 입력 필드 |
| `border` | `#e2e8f0` | 테두리·구분선 |
| `ink` / `ink-sub` / `ink-muted` | `#0f172a` / `#475569` / `#94a3b8` | 제목·수치 / 본문 / 라벨·축 |
| `brand` / `brand-tint` | `#2563eb` / `#eff6ff` | 강조·활성 / 배지·칩 배경 |
| `urgent` / `urgent-tint` | `#ea580c` / `#fff7ed` | D-3 이내·접수중 |
| `up` / `down` | `#0d9488` / `#dc2626` | 상승 / 하락 변동률 |
| `line` / `line-base` | `#2563eb` / `#e2e8f0` | 지수 라인 / 기준선 100 |

모서리는 `chip 6` / `field 8` / `cell 12` / `card 16` / `hero 20`. 폰트는 **Gothic A1**.

### 규칙

- **숫자·금액은 우측 정렬** + `tabular-nums`. Gothic A1은 숫자 자리폭이 고르지 않아 필수다.
- **금액은 억/만 단위로 축약**: `27600` → `"2억 7,600만"`.
- **면적은 소수 1자리까지만 표시한다.** 상류가 `112.4279`처럼 넷째 자리까지 주는데 표에서
  자릿수가 들쭉날쭉해 읽기 어렵다. 값은 원본을 쓰고 표시만 줄인다.
- **D-day 배지는 진행 중일 때만 색을 쓴다.** D-3 이내 주황 틴트, 그 외 진행 중은 브랜드 틴트,
  **마감·일정 미정은 중립.** 마감까지 색을 주면 목록에서 전부 같은 무게로 보여 아직 넣을 수
  있는 공고가 묻힌다.
- **지수 차트는 단색 라인 + 기준선(100).** 대시보드 카드는 `minimal` 모드로 축·격자를 지우고
  선만 남기되 기준선은 유지한다.
- **상승/하락 변동률만 색을 쓴다**(`up`/`down`). 시리즈 구분에 재사용하지 않는다.
- 지수는 기준시점 100 기반이므로 **절대 가격이 아니라는 각주**를 차트마다 붙인다.
- **값이 없는 칸·칩은 렌더하지 않는다.** `-`를 줄줄이 늘어놓지 않는다(§4.4와 같은 원칙).
  유형마다 있는 필드가 달라서(시공사는 APT 전용, 공급면적은 OPT에 없음) 빈 칸이 흔하다.

예외: RegionMarket은 매매·전세·실거래 3개 라인을 겹치므로 상색 + 실선·점선 패턴으로 구분한다(§9).
...
| 디자인 | 구현 | 이유 |
|---|---|---|
| 헤더 "청약 가이드", 배너 가이드 링크 | 넣지 않음. 배너는 가점 계산기로 | 그 화면이 없다 — 없는 곳으로 링크하지 않는다 |
| 시세 차트(절대좌표 SVG) | recharts `minimal` 모드 | 데이터 기반이라 정적 SVG로 대체 불가 |
| "일반공급 자격 요건" 3줄 | 규제 플래그 + 공고문 링크 | 청약홈 API에 자격 요건 필드가 없다. 하드코딩하면 틀린 정보가 된다(§4.0) |
| 상세에 없는 섹션들 | 디자인 섹션 아래에 유지 | 주택형 표·특별공급·지도·지역 시세는 Phase 4~7의 기능이다. 안 그려졌다는 것이 삭제 근거는 아니다 |
...
formatArea(112.4279)       // "112.4㎡"  — 표시만 소수 1자리
formatPhone('15990199')    // "1599-0199"
formatDday(3)              // "D-3" / 0 → "D-DAY" / 음수 → "마감"
formatChange(-0.12)        // "▼ 0.12%"  — 화살표를 함수가 붙인다(중복 금지)
...
세 시리즈 모두 통계표 코드가 확정돼 정상 상태에서는 3라인이 다 나온다. 한 시리즈만 비면
그 라인만 빠지고, **섹션 전체가 사라지면 틀렸다** — 나머지 라인은 나와야 한다.
...
**실패를 안내 문구로 채우지 않는 이유**(다시 열지 말 것): 두 실패의 청중이 다르다.
지오코딩 실패(`ZERO_RESULT`)는 데이터 조건이라 사용자가 할 수 있는 일이 없고 "위치를 찾을 수
없습니다"는 정보량이 0이다. 도메인 미등록(ORB)은 설정 오류라 **운영자**에게는 진단이 필요하지만
사용자에게는 역시 할 수 있는 일이 없다. 그래서 운영자용 진단은 화면이 아니라 `DEPLOY.md`가
담당한다 — ORB 증상 설명과 원인을 가르는 curl 한 줄이 거기에 있다.

#### 주소 전처리 — 무엇이 실제로 성공을 만드는가 (2026-09-24 실측)

**측정은 반드시 브라우저 SDK를 통해야 한다.** JS 키로 REST `dapi.kakao.com/v2/local/search/address.json`을
직접 호출하면 `401 {"errorType":"AccessDeniedError","message":"cannot find Authorization : KakaoAK header"}`이
돌아오는데, 본문을 읽지 않고 `documents` 길이만 보면 **모든 주소가 0건으로 보인다**. §4.5①의
"정상 응답처럼 보이는 실패"와 같은 부류다. 아래 표는 전부 실제 브라우저 SDK로 측정했다.

| 입력 | 결과 |
|---|---|
| `경기도 시흥시 하중동 일원 (…A-4블록)` (원문) | `ZERO_RESULT` |
| `경기도 시흥시 하중동 일원` (괄호만 제거) | `ZERO_RESULT` |
| `경기도 시흥시 하중동 (…A-4블록)` (`일원`만 제거) | **`OK`** |
| `경기도 시흥시 하중동` (둘 다 — 현재 구현) | **`OK`** |

**지오코딩을 막는 것은 `일원`이다 — 단, 번지가 없을 때만.** 카카오는 말미 괄호를 스스로
무시하고(`하중동 (A블록) (임대)`, `명지동 (명지국제신도시)` 모두 원문 그대로 `OK`),
**번지가 있으면 `일원`도 무시한다.**

| 형태 | 관측 | 원문 | 전처리 후 |
|---|---|---|---|
| `동 + 일원` (번지 없음) — `경기도 시흥시 하중동 일원 (…)` | **1건** | `ZERO_RESULT` | `OK` |
| `동 + 번지 + 일원` — 광진·미추홀·제물포 | **3건** | **`OK`** | `OK` |
| `동 + 번지` — `부산광역시 강서구 범방동 2008번지` | **1건** | `OK` | `OK` |

즉 전처리가 결과를 바꾸는 것은 **번지 없이 `일원`이 붙은 형태 하나뿐**이다. 나머지는
전처리 유무와 무관하게 성공한다(전처리가 해를 끼치지도 않는다 — 번지를 남기므로 같은 지점을 찾는다).

괄호를 **먼저** 떼는 이유는 `일원$` 정규식이 앵커이기 때문이다. 괄호가 뒤에 붙어 있으면
`일원`이 말미에 노출되지 않아 제거 자체가 불가능하다.
**괄호 제거는 `일원` 제거의 전제이지 그 자체로 성공시키는 단계가 아니다. 순서를 바꾸면 깨진다.**

**반복 제거 루프가 결정적인 경우**: `일원` 뒤에 괄호가 2개 이상일 때.
`경기도 시흥시 하중동 일원 (A블록) (임대)` → 1회 제거는 `ZERO_RESULT`, 반복 제거는 `OK`.
이 형태가 실데이터에 있는지는 미확인이다.

**`일원` 제거가 지명을 깎지 않는다**: `서울특별시 강남구 일원동`은 `일원$`에 걸리지 않아
원문 그대로 통과하고 `OK`다.

**표본 — Phase 8 실데이터로 갱신**:

Phase 8에서 픽스처가 실측 공고로 바뀌면서 `일원` 주소가 1건에서 4건이 됐고, 그 결과
위의 "번지 유무" 분기가 드러났다.

| 실제 주소 | 형태 | 원문 검색 |
|---|---|---|
| `경기도 시흥시 하중동 일원 (시흥하중 …A-4블록)` | 동 + 일원 | `ZERO_RESULT` |
| `서울특별시 광진구 구의동 592-39번지 일원` | 동 + 번지 + 일원 | `OK` |
| `인천광역시 미추홀구 숭의동 350-1번지 일원` | 동 + 번지 + 일원 | `OK` |
| `인천광역시 제물포구 송림동 31-3번지 일원` | 동 + 번지 + 일원 | `OK` |

**전처리가 결과를 바꾼 실제 주소는 여전히 1건(시흥)뿐이다.** `번지 일원` 3건은 전처리가
없어도 성공한다. Phase 1~7 문서가 "전처리가 없으면 지구 단위 공고 지도가 전부 사라진다"고
쓴 것은 이 분기를 모르고 한 과장이었다.

`경기도 화성시 동탄면 일원`·`하중동 (A블록) (임대)`·`명지동 (명지국제신도시)`는 기전 확인용
**합성 문자열**이다 — 실데이터 표본에 세지 말 것.

**픽스처 주의**: Phase 8에서 픽스처가 실측 공고로 바뀌면서 `판교역로 123`(실재하지 않는
번지라 지도가 숨던 케이스)은 사라졌다. 이제 지도가 안 뜨는 픽스처는 `test/fixtures/dev/` 2건
(`개발용 픽스처`라는 가짜 주소)뿐이고, 이건 실패 경로를 화면에서 보기 위한 의도된 케이스다.

#### 좌표 초기화와 취소 플래그가 발화하는 경로 (2026-09-24 돌연변이 테스트로 확정)

`KakaoMap`의 `setCoords(null)`과 `cancelled` 플래그는 **공고 간 이동이 아니라 리페치로** 발화한다.
이 구분을 문서에 남기는 이유는, 같은 조사를 세 번 하지 않기 위해서다.

- **공고 이동으로는 발화하지 않는다.** 전환 때마다 `KakaoMap`이 언마운트→재마운트되고(마운트
  프로브로 확인), 새 인스턴스의 `coords` 초깃값이 이미 `null`이다. 두 코드를 각각 제거해도
  공고 전환 시나리오는 그대로 통과한다. 그리고 이 앱에는 상세→상세 링크가 없어 상세 간
  직접 전환 자체가 만들어지지 않는다.
- **리페치로는 발화한다.** `useNoticeDetail`은 `refetchOnWindowFocus`를 끄지 않았고
  `QueryClient`에도 `defaultOptions`가 없다 → 기본 활성이다. 상세를 열어둔 채 staleTime(30분)이
  지나 재포커스되면 리페치가 돌고, 정정공고로 `HSSPLY_ADRES`가 바뀌면 **언마운트 없이 prop만**
  갱신된다.

돌연변이 테스트 결과(상세 API 응답의 주소를 2회차부터 바꾸고 `window`에 `visibilitychange`를 쏴
네비게이션 없이 리페치를 유발):

| 제거한 코드 | 결과 |
|---|---|
| `setCoords(null)` | **정정 전 시흥 지도가 강남 주소 공고에 그대로 남는다** |
| `cancelled` | 1회차 지오코딩을 지연시키면, **늦게 도착한 시흥 좌표가 정정된 화면에 지도를 심는다** |

둘 다 §9의 "없는 것보다 틀린 것이 나쁘다"를 정확히 위반하는 상태다. **방어 코드가 아니라
동작 코드다** — 제거하지 말 것.

**대안으로 검토하고 기각한 것**: 부모에서 `<KakaoMap key={notice.address} …>`로 강제 재마운트해
두 줄을 없애는 방법. 실제로 적용해 위 두 시나리오를 돌려봤고 **둘 다 통과한다** — 정확성으로는
동등하다. 그럼에도 현재 구조를 유지하는 이유는 고장 반경이다. `key`는 리스트 렌더링 관용구로
보여 리팩터링 때 "불필요해 보이는 prop"으로 지워지기 쉽고, 지워지는 순간 두 보호가 **동시에**
사라지는데 `KakaoMap.tsx` 안에는 그 단서가 남지 않는다. 반면 지금 구조는 보호가 그것을 필요로
하는 state 옆에 있고 주석이 발화 경로를 명시한다. 정확성이 같을 때는 이쪽을 고른다.

도로명 주소도 정상 동작한다(`경기도 성남시 분당구 판교역로 235` → `OK`). 번지가 실재하지
않으면 `ZERO_RESULT`이고(`판교역로 123`), 이때 섹션이 숨는 것이 설계대로다.
...
**확인 결과** (2026-09-24, 전부 닫힘):

| 항목 | 결과 |
|---|---|
| `URBTY_OFCTL`/`PBL_PVT_RENT`/`OPT` 실제 payload | ✅ **필드명 전부 일치.** OAS 기반 합성이 정확했다 |
| `CMPET_RATE`의 비수치 값 | ✅ **실재한다.** 괄호까지 붙는 `"(△9)"` — `rateRaw` 경로가 필요했다 |
| `HOUSE_TY`가 (A)(B)에서 일치하는지 | ✅ **원문 그대로 일치**(300/300·249/249·291/291). 정규화는 이득 0·충돌 10건이라 제거(§4.3⑥) |
| 실거래가격지수 `STATBL_ID` | ✅ `A_2024_00178` — 통계표 목록 엔드포인트로 확보 |
| 시도별 `CLS_ID` 17개 | ✅ **통계표마다 다르다.** 매매·전세는 같고 실거래는 별도 체계 — `clsIdFor(region, statblId)`로 분리 |
| 전남광주통합 반영 | ✅ 광주·전남의 `CLS_FULLNM`이 `"전남광주>광주"`로 내려갔다. `CLS_ID` 직접 매핑이라 영향 없음 |
| `START_WRTTIME`/`END_WRTTIME` | ✅ **동작한다**(§4.5⑤) |

Phase 8이 추가로 드러낸 것:

- `API-FIELDS.md`의 `"55㎡O"` 기록이 **웹사이트 렌더링을 API 실측으로 잘못 옮긴 것**이었고,
  그 오기가 픽스처·정규화 함수·테스트까지 전파돼 있었다.
- `GP`(군)가 없으면 빈 문자열이 아니라 `"-"`로 온다 — 주택형 열에 `"- 59A-1"`이 나가고 있었다.
- `getAPTSpsplyReqstStus`는 **주택형별 다행**이다. 첫 행만 쓰고 있어 특별공급 수치가
  공고 전체의 1/6로 표시됐다.
- 샘플 감지 기준이 공표 지연을 오탐했다(§4.5①).
...
[카카오 키 축 — 완료 (2026-09-24)]
  지오코딩 성공 경로 검증                              ✔ 실브라우저(Playwright + Chrome)
  ├ 성공: 픽스처 주소 4건(시흥·청주·위례·동탄)에서 마커·타일 렌더까지 확인
  ├ 실패(ZERO_RESULT): 섹션 자체가 숨는다 — 빈 지도 박스가 남지 않음
  ├ 전처리의 인과를 분해: `일원` 제거가 결정적, 괄호 제거는 그 전제 (§9 표)
  ├ 도메인 제한은 포트까지 검사. 거부 증상은 ERR_BLOCKED_BY_ORB (DEPLOY.md)
  └ 프로덕션 빌드에서도 동일(NEXT_PUBLIC_* 인라인 확인, 서버 키 값 유출 없음)

  ├ setCoords(null)·cancelled가 발화하는 경로를 확정 — 리페치다(§9). 둘 다
    돌연변이가 죽는다. "공고 전환이 막아준다"는 Phase 7의 전제는 반증됐다

  검증했다고 말할 수 없는 것 — 구별력이 없었던 관측:
  ├ 공고 전환 성공→성공: 수정이 없어도 새 좌표가 덮어써서 최종 상태가 같다
  ├ 전환 전이 구간: 지오코딩이 수백 ms라 "이전 지도가 잠깐 보이는 창"은 미관측
  └ https end-to-end: SDK가 location.protocol을 따르므로 mixed content 위험은
    없지만(로더 소스 확인), https에서 끝까지 태워본 적은 없다 → 배포 후 첫 확인

  남은 것: `일원` 뒤에 괄호가 2개 이상인 실주소 — 실데이터가 있어야 확인 가능
